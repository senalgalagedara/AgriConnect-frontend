...
export function logApiDiagnostics() {
  ...
  const sessionPath = `${API_PATH_PREFIX || ''}/auth/session`.replace(/\/+/g,'/');
  console.table({
    API_BASE_URL,
    API_PATH_PREFIX: API_PATH_PREFIX || '(none)',
    computedSessionURL: `${API_BASE_URL}${sessionPath.startsWith('/') ? '' : '/'}${sessionPath}`,
    locationOrigin: window.location.origin,
    sameOrigin: window.location.origin.startsWith(API_BASE_URL)
  });
  console.info('[diagnostics] If sameOrigin=false you need proper CORS + credentials.');
}
